...
<details>
<summary>Q1. 계약 테스트는 정확히 무엇을 잡고, 무엇을 못 잡는가?</summary>

잡는 것: 인터페이스의 **모양**(필드 존재·타입)이 프로듀서·소비자 양쪽에서 일치하는지를 **배포 전 빌드 시점**에 잡는다. §7.3에서 실측했듯, 소비자가 누락된 필드를 `int` 기본값 `0`으로 조용히 삼키던 결함을 계약 테스트가 실제로 잡아냈다. 못 잡는 것: **브로커까지의 실제 왕복**(파티션 키·헤더·릴레이 동작, §4.5) — 이벤트 계약은 Kafka 를 거치지 않고 outbox 발신함의 페이로드까지만 대조한다. DB·Kafka·Eureka 를 아우르는 통합 신뢰성은 여전히 Phase 12~14 의 compose 실증이 담당한다.
</details>

<details>
<summary>Q2. "계약은 값이 아니라 모양을 못 박아야 한다"(§6.3)는 말은 무슨 뜻인가?</summary>

`availableQuantity: 100` 처럼 구체적인 값을 계약하면, 프로듀서가 테스트 시드 데이터만 바꿔도 계약이 깨진다 — 계약이 인터페이스가 아니라 우연히 그 순간의 값에 결합돼 버린다. `type: by_regex, predefined: number` 처럼 **타입/형식만** 계약하면 실제 값이 바뀌어도 계약은 안정적으로 유지된다.
</details>

<details>
<summary>Q3. 스키마 진화의 네 가지 변경(§4.6) 중 "개명(rename)"이 가장 위험한 이유는?</summary>

타입 변경은 `InvalidFormatException`을 던져 시끄럽게 실패하고 DLT에 증거를 남긴다 — 실패가 눈에 보인다. 하지만 개명은 예외 없이 값만 `null`이 된다 — 시스템은 정상 동작하는 것처럼 보이면서 **조용히 데이터를 잃는다**. "개명 = 제거 + 추가"이므로, 안전하게 하려면 "새 필드 추가 → 소비자 전환 → 옛 필드 제거"의 3단계를 거쳐야 한다.
</details>

<details>
<summary>Q4. Tolerant reader가 "관대함"에서 "데이터 유실"로 넘어가는 경계는 어디인가?</summary>

Tolerant reader는 "**모르는** 필드를 무시한다"는 뜻이지 "**필요한** 필드가 없어도 괜찮다"는 뜻이 아니다. §7.3의 결함이 정확히 이 경계를 넘은 사례다 — 필수 필드가 `int`(원시타입)였기 때문에 필드가 사라지자 Jackson이 `0`이라는 그럴듯한 거짓값으로 채워 조용히 흘려보냈다. `Integer`(박싱 타입)로 받고 `null`을 명시적으로 거부해야 관대함이 유실로 번지지 않는다.
</details>
...
| **하위 호환(backward compatible)** | 새 버전의 프로듀서가 **옛 소비자**와도 문제없이 동작한다(예: 필드 추가 — 옛 소비자는 모르는 필드를 무시한다) |
| **상위 호환(forward compatible)** | 옛 버전의 프로듀서가 **새 소비자**와도 문제없이 동작한다(예: 리플레이로 되살아난 과거 이벤트를 새 소비자가 null 허용 등으로 견딘다) |
