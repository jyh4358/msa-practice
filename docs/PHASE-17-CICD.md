...
>
> ⚠️ **정정(요지 — 이후 변경, 현재 파일 참고).** 위 코드 블록은 Phase 17 시점의 것이다. Phase 18 에서
> `apply.sh` 가 오버레이 인자를 받도록 바뀌면서, 지금 CI 는 `run: ./deploy/k8s/apply.sh ci` 로
> **오버레이를 명시**해 호출한다(인자를 생략하면 기본값인 `local` 오버레이가 적용되어 로컬 이미지를
> 찾다가 깨진다). 위 `sed` 도 Phase 18 이후 Kustomize 의 `images:` 변환기로 대체됐다
> ([PHASE-18-KUSTOMIZE.md](PHASE-18-KUSTOMIZE.md) §4-④).
...
`GITHUB_TOKEN` 은 실행이 끝나면 폐기되는 임시 토큰이다. 잡마다 필요한 만큼만 권한을 준다 —
이미지 잡에만 `packages: write`, 스모크 잡은 pull 만 하므로 `packages: read` 다.

> ⚠️ **정정(2026-08-02 감사).** 이 절은 원래 "`GITHUB_TOKEN` 의 기본값은 읽기 전용이고" 라고 적었는데
> 그건 부정확했다. `permissions` 를 아예 안 적은 잡은 **저장소 설정의 기본값**을 물려받고, 그 기본값은
> 저장소마다 다르게 설정할 수 있다(조직 정책에 따라 read/write 가 기본일 수도 있다) — **보장되지 않는다.**
> 감사는 워크플로 최상단에 `permissions: { contents: read }` 를 명시적으로 못 박아, `permissions` 를
> 안 적은 잡(`빌드·테스트` 등)도 항상 읽기 전용에서 시작하도록 고쳤다(현재 `ci.yml` 48~49행).
> 이유: `./gradlew build` 가 실행하는 서드파티 코드(Gradle 플러그인·Testcontainers)가 저장소 기본값이
> read/write 인 경우 이론상 `main` 에 push 할 수 있었고, Phase 19 의 Argo CD `selfHeal` 이 `main` 을
> 그대로 신뢰하므로 이는 곧 클러스터 장악 경로가 될 수 있었다.
...
| 2 | **CD 가 없다 — 배포는 여전히 수동** | CI 는 "배포 가능함"만 증명한다. 실제 클러스터에 반영하는 건 사람이다 | **Phase 19**(Argo CD 로 GitOps 도입 — **정정**: Phase 18 은 Kustomize/Helm 전환만 다루고 이 항목은 다루지 않았다) |
...
| 7 | **버전이 `0.0.1-SNAPSHOT` 고정** | 릴리스 개념이 없다. 태그·체인지로그·시맨틱 버저닝 부재 | 미해결 — **정정**: Phase 18·19 모두 이 항목을 다루지 않았다 → [BACKLOG.md](BACKLOG.md) (SemVer/릴리스 프로세스) |
...
<details><summary>Q1. "빌드는 한 번, 포장은 아키텍처별"은 무슨 뜻이고 왜 그렇게 하나?</summary>

Java 바이트코드는 CPU 아키텍처를 타지 않는다 — 아키텍처마다 달라지는 건 **베이스 이미지(JRE)** 뿐이다.
그래서 컴파일·테스트(87개)는 `빌드·테스트` 잡에서 **한 번만** 돌리고 jar 을 artifact 로 넘긴 뒤,
`이미지(amd64)`·`이미지(arm64)` 두 잡은 그 jar 을 각자 네이티브 러너에서 **포장만** 한다(§4-①).
컴파일을 두 번 하지 않아 시간이 절반이고, 두 아키텍처가 **정확히 같은 코드**임이 보장된다.
</details>

<details><summary>Q2. 이미지 태그가 커밋 SHA 라는 것은 무엇을 보장하나?</summary>

`:latest` 만 쓰면 "지금 클러스터에 떠 있는 게 어느 커밋인지" 아무도 모른다(§1-①, Phase 16 의 문제).
`:<커밋SHA>` 태그는 **떠 있는 이미지 → 소스 커밋**을 1:1 로 잇는다 — 버그를 발견했을 때 그 이미지가
어느 소스로 만들어졌는지 즉시 답할 수 있고, 롤백도 "그 커밋 태그로 되돌아가기"로 명확해진다(§3).
</details>

<details><summary>Q3. 잡을 나누면 "파일이 공유되지 않는다"는 게 왜 문제가 되고, 어떻게 해결하나?</summary>

한 워크플로 안이라도 **잡마다 새 VM 을 받는다**(§6-①). `빌드·테스트` 잡이 만든 jar 은
`이미지` 잡의 파일시스템에 없다 — 그래서 `upload-artifact`/`download-artifact` 로 명시적으로 넘긴다.
같은 잡 안의 **스텝**끼리는 파일이 공유된다는 점과 대비된다.
</details>

<details><summary>Q4. CI 가 빨라지면서 오히려 실패한 사건(§7-⑦)은 무엇을 말해주나?</summary>

느릴 때는 이미지 pull·의존성 다운로드가 시간을 벌어줘 admission webhook 준비를 기다리는 경쟁을
우연히 피해 갔다. Gradle 캐시로 빨라지자 그 창에 정확히 들어가 실패했다 — **성능 개선이 잠재
결함을 드러낸** 사례다. 로컬에서는 사람이 명령을 하나씩 치는 동안 시간이 흐르므로 이 경쟁을 절대
볼 수 없다는 점도 함께 기억할 것.
</details>
