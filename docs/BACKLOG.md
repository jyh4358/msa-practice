...
| 격리(quarantine)된 outbox 행 재처리 도구 | Phase 14가 `attempts` 상한으로 outbox row를 격리는 하지만, 격리된 뒤 재투입하는 도구가 없어 사람이 수동 개입해야 한다 | 각 Phase 문서에서 이미 "운영 도구 영역"으로 남긴 항목 — DLQ 재투입 도구(PHASE-13/14 §8)와 같은 갈래 |
